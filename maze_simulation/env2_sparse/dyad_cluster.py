...
class BodaBorg:
    ...
    def trans_prob(self, S_weekly, A_weekly, S_daily, A_daily):
        ...
        move_correct_prob = [0.9, 0.6] ##sunny, rainy
    ...
    def reward_function(self, S_weekly, A_weekly, S_daily, A_daily, S_daily_next):
        rewards = [1,1.2]
        reward = (S_daily_next[0] - S_daily[0])*rewards[A_weekly]*(S_daily[0]==1)
    ...
    def value_iteration(self):
        ...
        for h in reversed(range(self.H+1)):
            for S_weekly, S_daily_x, S_daily_y, A_weekly in\
                    itertools.product(range(self.S_weekly_d), range(self.S_daily_d[0]), range(self.S_daily_d[1]), range(2)):
                if h == self.H:
                    V_func_daily[(h, S_weekly, A_weekly, S_daily_x, S_daily_y)] = 0.0
                else:
                    Q_func = [0.0, 0.0]
                    for action in range(2):
                        prob_axes = self.trans_prob(S_weekly, A_weekly, [S_daily_x, S_daily_y], action)
                        Q_temp = 0.0
                        for S_daily_x_next, S_daily_y_next in itertools.product(range(self.S_daily_d[0]), range(self.S_daily_d[1])):
                            Q_temp += prob_axes[S_daily_x_next, S_daily_y_next]*V_func_daily[(h+1, S_weekly, A_weekly, S_daily_x_next,S_daily_y_next)]
                            Q_temp += prob_axes[S_daily_x_next, S_daily_y_next]*self.reward_function(S_weekly, A_weekly, [S_daily_x, S_daily_y], action, [S_daily_x_next,S_daily_y_next])
                        Q_func[action] = Q_temp
                    opt_policy[(h, S_weekly, A_weekly, S_daily_x,S_daily_y)] = np.argmax(Q_func)
                    V_func_daily[(h, S_weekly, A_weekly, S_daily_x,S_daily_y)] = max(Q_func)
                    

        ## weekly value function and optimal policy
        V_func_weekly = {}
        opt_policy_week = {}
        for S_weekly in range(self.S_weekly_d):
            Q_func_week = [0.0, 0.0]
            for action in range(2):
                Q_func_week[action] = V_func_daily[(0, S_weekly, action, 0, 0)]
            opt_policy_week[S_weekly] = np.argmax(Q_func_week)
            V_func_weekly[S_weekly] = max(Q_func_week)

        return(opt_policy, V_func_daily, opt_policy_week, V_func_weekly)
...
## A sample player
player0 = BodaBorg()
opt_policy, value_fun, opt_policy_week, value_fun_week = player0.value_iteration()
...
for iter in range(N):
    ...
    opt_exp_reward = np.array([value_fun_week[i] for i in dyadRL1.weekly_states])
    regrets_this = opt_exp_reward - sum_of_rewards
    regrets = np.append(regrets, [regrets_this], axis = 0)

average_rewards = np.mean(rewards, axis = 0)
average_regrets = np.mean(regrets, axis = 0)
cumulative_regret = np.cumsum(average_regrets)
...
for iter in range(N):
    ...
    opt_exp_reward = np.array([value_fun_week[i] for i in dyadRL1.weekly_states])
    regrets_this = opt_exp_reward - sum_of_rewards
    regrets_full = np.append(regrets_full, [regrets_this], axis = 0)

average_rewards_full = np.mean(rewards_full, axis = 0)
average_regrets_full = np.mean(regrets_full, axis = 0)
...
for iter in range(N):
    ...
    opt_exp_reward = np.array([value_fun_week[i] for i in dyadRL1.weekly_states])
    regrets_this = opt_exp_reward - sum_of_rewards
    regrets_bandit = np.append(regrets_bandit, [regrets_this], axis = 0)

average_rewards_bandit = np.mean(rewards_bandit, axis = 0)
average_regrets_bandit = np.mean(regrets_bandit, axis = 0)
...
for iter in range(N):
    ...
    opt_exp_reward = np.array([value_fun_week[i] for i in dyadRL1.weekly_states])
    regrets_this = opt_exp_reward - sum_of_rewards
    regrets_stan = np.append(regrets_stan, [regrets_this], axis = 0)

average_rewards_stan = np.mean(rewards_stan, axis = 0)
average_regrets_stan = np.mean(regrets_stan, axis = 0)
...
with open(filename, 'wb') as f:  # Python 3: open(..., 'wb')
    pickle.dump([rewards, regrets, \
                 rewards_full, regrets_full, \
                 rewards_bandit, regrets_bandit, \
                 rewards_stan, regrets_stan], f)
